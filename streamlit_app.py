...
st.sidebar.markdown("---")
st.markdown("<h1 style='text-align:center;'>🚗 QR Car Management</h1>", unsafe_allow_html=True)
...
menu = [
    "📋 Xem danh sách", "🔍 Tìm kiếm xe", "➕ Đăng ký xe mới", "✏️ Cập nhật xe",
    "🗑️ Xóa xe", "📱 Mã QR xe", "📤 Xuất ra Excel", "📊 Thống kê xe theo đơn vị"
]
...
if choice == "📋 Xem danh sách":
    ...
    if bien_so in bien_so_da_dang_ky.values:
        ...
        if st.button("📥 Đăng ký"):
            try:
                sheet.append_row([
                    len(df) + 1, ho_ten, bien_so, ma_the, ma_don_vi,
        ...
        if ket_qua.empty:
            ...
            if st.button("Cập nhật"):
                sheet.update(f"A{index+2}:I{index+2}", [[
                    row["STT"], ho_ten_moi, bien_so_moi, row["Mã thẻ"],
                    ma_don_vi_moi, ten_don_vi_moi, chuc_vu_moi,
                    so_dien_thoai_moi, email_moi
                ]])
